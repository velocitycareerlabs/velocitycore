...
const { first, map, reduce } = require('lodash/fp');
  ...
  describe('Metadata list resolution test group', () => {
    ...
    describe('Resolve did document', () => {
      ...
      it('Create and resolve multi did', async () => {
        ...
        const indexEntries = [
          [primaryAddress, credentialMetadata.listId, 2342],
          [primaryAddress, credentialMetadata.listId, 2342],
        ];
        const did = buildMultiDid(indexEntries);
        const didDocument =
      ...
      it('Unsupported version fails to resolve multi did', async () => {
        const listId = 12;
        await operatorMetadataRegistryClient.createCredentialMetadataList(
          listId,
          vc,
        ...
        await expect(result).rejects.toThrow('Unsupported version "0xe629"');
      });

      it('Unsupported encryption algorithm fails to resolve multi did', async () => {
        const listId = 11;
        await operatorMetadataRegistryClient.createCredentialMetadataList(
          primaryAddress,
          listId,
