...
const apiUrl = mainConfig.registrarApi;
const apiOrigin = new URL(mainConfig.registrarApi).origin;
  ...
  getList: (resource, params) => {
    ...
    switch (resource) {
      ...
      case 'reference/countries': {
        return httpClient(`${apiOrigin}/${resource}`).then(({ json }) => {
          return {
            data: json.map((item) => {
    ...
    return httpClient(fullUrl).then(({ json }) => {
      return {
        data: json.result,
  ...
  getOne: (resource, params) => {
    ...
    switch (resource) {
      ...
      case dataResources.INVITATIONS: {
        ...
        if (params.meta.code) {
          return httpClient(`${apiUrl}/${resource}/${params.meta.code}`).then(({ json }) => {
