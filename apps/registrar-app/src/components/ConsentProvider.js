...
import { TERMS_AND_CONDITIONS_VERSION } from '../pages/TermsAndConditions';
import TermsAndConditionsPdf from '../pages/TermsAndConditionsPdf';
...
const ConsentProvider = ({ children }) => {
  ...
  const [isShouldAgreeWithNewVersion, setIsShouldAgreeWithNewVersion] = useState(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setIsLoading(true);
        const response = await fetch(`${mainConfig.registrarApi}/${dataResources.CONSENTS}`, {
          method: 'GET',
          headers: {
  ...
  const onAgreeWithNewVersion = async () => {
    try {
      const token = await getAccessToken();
      await fetch(`${mainConfig.registrarApi}/${dataResources.CONSENTS}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Accept: 'application/json',
